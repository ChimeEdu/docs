...
description: "Issue a ChimeEdu map, timeline, chart, or diagram as one shared copy, one per student, or one per group — and track who has started work."
keywords: ["issuance", "per-student", "group assignment", "assign activity", "student copy", "roster", "groups", "diagram"]
---
...
<CardGroup cols={3}>
  <Card title="One shared" icon="users">
    Everyone edits the same map, timeline, chart, or diagram — the classic flow, and the default for every activity.
  </Card>
...
<Steps>
  ...
  <Step title="Choose a mode">
    In the activity's settings, pick **Per-Student** or **Group Assignment** under **Issuance**. For a diagram, the choice is under **Who edits this diagram?** on the diagram's setup page.
  </Step>
...
- Anything you put on the activity before issuing it stays visible to everyone as a shared starting layer. On maps, your own pins show for every student but are **locked** — students can read them, but only you can change them. On diagrams, what you draw on your own canvas is read-only reference material in each student's copy.
...
To look at a student's work, open their entry from the panel. Their copy — or their group's — opens **read-only**: you see everything they've done, but you can't change a student's or group's work. The only copy you edit is your own shared starting layer, so open the activity itself (not a student's copy) to change that. On charts there's no per-student open link yet; use **Live Results** to see the class.
