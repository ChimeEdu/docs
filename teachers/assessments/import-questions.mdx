...
| `type` | *Assessments only, optional.* `multiple_choice`, `true_false`, `short_answer` or `long_answer`. Leave it out and ChimeEdu works it out from the row: two options reading True/False → true/false; any other options → multiple choice; no options → short answer |
...
- **One correct answer per question** for now. Rows with several correct options are skipped until multi-select questions arrive.
